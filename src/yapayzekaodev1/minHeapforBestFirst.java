...
public class minHeapforBestFirst {
        public ArrayList <Node> Heap; 
    public int size; 
    public int maxsize; 
    private static final int FRONT = 1; 
  
    public minHeapforBestFirst(int maxsize) 
    { 
    ...
    {         
        ...
        if (!isLeaf(pos)) { 
            if (Heap.get(pos).h > Heap.get(leftChild(pos)).h 
                || Heap.get(pos).h > Heap.get(rightChild(pos)).h) { 
  
                // Swap with the left child and heapify 
                // the left child 
                if (Heap.get(leftChild(pos)).h < Heap.get(rightChild(pos)).h) { 
    ...
    {   
        ...
        int current = size; 
        while (Heap.get(current).h < Heap.get(parent(current)).h) {            
